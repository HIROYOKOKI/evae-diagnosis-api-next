...
export default function SoulProfileForm() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // レスポンスの処理
      // レスポンスがOKでない場合もエラーとして処理
      if (!res.ok) {
        const errorText = await res.text(); // エラーレスポンスの内容を確認
    ...
    } catch (error: any) { // errorの型をanyにキャスト
      console.error("プロフィール診断エラー:", error);
      // 例: エラーメッセージをユーザーに表示
      alert(`プロフィールの保存に失敗しました。詳細: ${error.message} もう一度お試しください。`); // 開発用のアラート
    }
  };
